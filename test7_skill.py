...
def _parse_frontmatter(text:str) -> tuple[dict, str]:
    ...
    if len(parts) < 3:
        return {}, text
    try:
        mata = yaml.safe_load(parts[1]) or {}
    except yaml.YAMLError:
        mata = {}
    return mata, parts[2].strip()


#在启动时构建技能注册表（用于在 load_skill 中进行安全查找）
...
# 07: SYSTEM includes skill catalog (cheap — just names + descriptions)
def build_system() -> str:
    "在启动时注入技能目录，以构建系统提示词（SYSTEM prompt）。"
    catalog = list_skills()
    return (
